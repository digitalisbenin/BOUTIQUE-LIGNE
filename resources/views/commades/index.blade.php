                                ...
                                <div>
                                    ...
                                    <nav aria-label="breadcrumb">
                                        <ol class="breadcrumb mb-0">
                                            ...
                                            <li class="" aria-current="page">Commande</li>
                                        </ol>
                                    </nav>
                                </div>
                                <!-- button -->
                                <div>
                                    <a href="{{ url('historique') }}" class="btn btn-primary">Historique
                                        </a>
                                </div>
                            </div>
                        </div>
...
@section('scripts')
<script>
    function searchLivreur(modalId) {
        let searchQuery = document.getElementById(`searchLivreur${modalId}`).value;
        if (searchQuery.length >= 3) { // Rechercher après 3 caractères
            fetch(`/search-livreur?query=${searchQuery}`)
                .then(response => response.json())
                .then(data => {
                    let livreurSelect = document.getElementById(`livreurSelect${modalId}`);
                    livreurSelect.innerHTML = '<option selected>sélectionner un livreur</option>';
                    data.livreurs.forEach(livreur => {
                        let option = document.createElement('option');
                        option.value = livreur.id;
                        option.textContent = `${livreur.nom} ${livreur.prenoms}`;
